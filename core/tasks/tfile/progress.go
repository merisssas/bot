...
func (p *Progress) OnStart(ctx context.Context, info TaskInfo) {
	...
	if ext != nil {
		if _, err := tgutil.EditMessage(ext, p.ChatID, req); err != nil {
...
func (p *Progress) OnProgress(ctx context.Context, info TaskInfo, downloaded, total int64) {
	...
	if !shouldUpdateProgress(total, downloaded, int(p.lastUpdatePercent.Load()), lastUpdateAt, now) {
		return
	}
	...
	if ext != nil {
		if _, err := tgutil.EditMessage(ext, p.ChatID, req); err != nil {
			log.FromContext(ctx).Errorf("Failed to send progress update message: %s", err)
		}
		return
	}
...
func (p *Progress) OnDone(ctx context.Context, info TaskInfo, err error) {
	...
	if ext != nil {
		if _, err := tgutil.EditMessage(ext, p.ChatID, req); err != nil {
			log.FromContext(ctx).Errorf("Failed to send progress done message: %s", err)
		}
	}
}
