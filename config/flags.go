...
func RegisterFlags(cmd *cobra.Command) {
	...
	flags.Bool("allow-remote-config", false, "allow loading config from remote http(s) URL")
	flags.String("remote-config-sha256", "", "expected sha256 checksum for remote config file")
	...
	flags.Bool("allow-self-update", false, "allow self-update from GitHub releases")
...
func bindFlags(cmd *cobra.Command) {
	...
	viper.BindPFlag("allow_remote_config", flags.Lookup("allow-remote-config"))
	viper.BindPFlag("remote_config_sha256", flags.Lookup("remote-config-sha256"))
	...
	viper.BindPFlag("allow_self_update", flags.Lookup("allow-self-update"))
