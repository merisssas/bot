...
var upgradeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		allowSelfUpdate, _ := cmd.Flags().GetBool("allow-self-update")
		if !allowSelfUpdate {
			fmt.Println("Self-update is disabled. Re-run with --allow-self-update to proceed.")
			return
		}
...
func init() {
	...
	upgradeCmd.Flags().Bool("allow-self-update", false, "allow self-update from GitHub releases")
}
