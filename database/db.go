...
func syncUsers(ctx context.Context) error {
	...
	if len(cfgUserMap) == 0 {
		logger.Warn("User sync skipped: config has no users; leaving database users unchanged")
		return nil
	}
	...
	for dbID, dbUser := range dbUserMap {
		if _, exists := cfgUserMap[dbID]; !exists {
			if !config.C().UserSync.DeleteMissing {
				logger.Warnf("User %d exists in database but not in config; skipping deletion because user_sync.delete_missing is false", dbID)
				continue
			}
