...
const (
	...
	DefaultUploadSize = 4000 * 524288 // 4000 * 512 KB
)
...
func (t *Telegram) Save(ctx context.Context, r io.Reader, storagePath string) error {
	...
	maxUploadSize := t.config.MaxUploadSizeMB * 1024 * 1024
	if maxUploadSize <= 0 {
		maxUploadSize = DefaultUploadSize
	}
	...
	if splitSize <= 0 {
		splitSize = maxUploadSize
	}
	if splitSize > maxUploadSize {
		splitSize = maxUploadSize
	}
