...
- `max_connection_per_server`: 单个服务器最大连接数，对只允许单连接的站点建议设置为 `1`
- `split`: 单个下载任务的分段数，对只允许单连接的站点建议设置为 `1`
- `min_split_size`: 分段最小大小（例如 `1M`）
- `continue`: 是否启用断点续传，默认 `true`
- `max_tries`: Aria2 重试次数，`0` 表示使用全局 `retry`
- `retry_wait_seconds`: 重试等待时间（秒），默认 `5`
- `poll_interval_seconds`: 下载状态轮询间隔（秒），默认 `2`
- `idle_timeout_minutes`: 无进展时自动中止的超时（分钟），`0` 表示禁用
- `min_free_space_mb`: 预留磁盘空间（MB），默认 `0`
- `progress_update_interval_seconds`: 进度消息更新间隔（秒），默认 `3`
- `startup_cleanup`: 启动时清理 `.aria2`/`.part` 残留文件
- `cleanup_dir`: 清理目录（为空时使用 Aria2 全局 `dir`）
