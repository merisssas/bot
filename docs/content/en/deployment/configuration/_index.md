...
- `max_connection_per_server`: Maximum connections per server, set to `1` for single-connection hosts
- `split`: Number of segments per download, set to `1` for single-connection hosts
- `min_split_size`: Minimum segment size (e.g. `1M`)
- `continue`: Enable HTTP resume support, default is `true`
- `max_tries`: Max retries for Aria2, `0` uses the global `retry` value
- `retry_wait_seconds`: Wait time between retries, default is `5`
- `poll_interval_seconds`: Polling interval for download status, default is `2`
- `idle_timeout_minutes`: Abort download if no progress within this time, `0` disables the check
- `min_free_space_mb`: Reserved free space (MB) to avoid filling the disk, default is `0`
- `progress_update_interval_seconds`: Progress message update interval, default is `3`
- `startup_cleanup`: Whether to remove leftover `.aria2`/`.part` files on startup
- `cleanup_dir`: Optional Aria2 download directory to scan for cleanup (empty uses Aria2 global `dir`)
