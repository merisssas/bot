...
import (
	...
	"github.com/merisssas/bot/common/utils/restart"
...
func handleUpdateCmd(ctx *ext.Context, u *ext.Update) error {
	if !config.C().AllowSelfUpdate {
		ctx.Reply(u, ext.ReplyTextString(i18n.T(i18nk.BotMsgUpdateErrorSelfUpdateDisabled, nil)), nil)
...
func handleUpdateCallback(ctx *ext.Context, u *ext.Update) error {
	if !config.C().AllowSelfUpdate {
		tgutil.EditMessage(ctx, u.GetUserChat().GetID(), &tg.MessagesEditMessageRequest{
			...
			Message: i18n.T(i18nk.BotMsgUpdateErrorSelfUpdateDisabled, nil),
		})
		return dispatcher.EndGroups
	}
	currentV, err := semver.Parse(config.Version)
