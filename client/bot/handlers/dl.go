...
func handleDlCmd(ctx *ext.Context, update *ext.Update) error {
	...
	for i, link := range links {
		normalized, err := netutil.ValidatePublicURL(context.Background(), link, netutil.AllowedSchemesDirect, false)
		if err != nil {
			logger.Warn("invalid link", "link", link, "error", err)
			links[i] = ""
			continue
		}
		links[i] = normalized
...
func handleAria2DlCmd(ctx *ext.Context, update *ext.Update) error {
	...
	links := args[1:]
	for i, link := range links {
		normalized, err := netutil.ValidatePublicURL(context.Background(), link, netutil.AllowedSchemesAria2, true)
		if err != nil {
			logger.Warn("invalid link", "link", link, "error", err)
			links[i] = ""
			continue
